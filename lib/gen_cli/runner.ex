defmodule GenCLI.Runner do
  ...
  def run(args) do
    ...
    gen_elements = GenCli.Parser.read_blueprint
    # Execute commands
    File.cd(generated_app_name)
    gen_commands = Enum.map(gen_elements, &GenCli.Parser.struct_to_command/1)
    Enum.each(gen_commands, &run_command/1) # TODO: change this to capture all output and show it at the end
  end
